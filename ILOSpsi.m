function [psi_d, x_e, y_e,xk_next,yk_next,xk_ahead,yk_ahead,stop] = ILOSpsi(x,y,Delta,h,R_switch,wpt,U,K_f,Gpi)
...
if isempty(k)
    ...
    xk_ahead = wpt.pos.x(k+1);
    yk_ahead = wpt.pos.y(k+1);
end
%% Read next waypoint (xk_next, yk_next) from wpt.pos 
n = length(wpt.pos.x);
if k < n-1                      % if there are more waypoints, read next one 
    xk_next = wpt.pos.x(k+1);  
    xk_ahead = wpt.pos.x(k+2);
    yk_ahead = wpt.pos.x(k+2);
    yk_next = wpt.pos.y(k+1);   
        ...
        xk_ahead = wpt.pos.x(n)+  R * cos(bearing);
        yk_ahead = wpt.pos.x(n) + R * cos(bearing);
end
