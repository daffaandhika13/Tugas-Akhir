function [psi_d, x_e, y_e,flag] = LOSpsi(x,y,Delta,h,R_switch,wpt,U,K_f,Kpi)
...
% using th
% e classical ILOS guidance law by Børhaug et al. (2008).
%
...
if isempty(k)
    ...
    min( sqrt( diff(wpt.pos.x).^2 + diff(wpt.pos.y).^2 ) )
...
n = length(wpt.pos.x);
% Switching untuk waypoint berikutnya
if k < n
    % Jika masih ada waypoint berikutnya, baca waypoint selanjutnya
    xk_next = wpt.pos.x(k+1);
    yk_next = wpt.pos.y(k+1);
    flag = 0;
else
        % Jika sudah melewati waypoint terakhir, lanjutkan dengan arah terakhir
...
% LOS guidance law
psi_d = psi_f;
psi_ref = pi_h - atan(Kpi(1)*y_e/Delta); 

% desired yaw rate r_d
Dy_e = -U * y_e / sqrt( Delta^2 + y_e^2 );           % d/dt y_e
r_d =-(1/Delta) * Dy_e / ( 1 + (y_e/Delta)^2 );      % d/dt psi_d
